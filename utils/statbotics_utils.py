...
# Statbotics Matches
############################################
def download_statbotics_matches(event: str, output_path: str, quals_only=True):
    """
...
def load_statbotics_matches(filename: str) -> pd.DataFrame:
    """
...
# Statbotics Events
############################################
def download_statbotics_event_teams(event: str, output_path: str):
    """
    ...
    sb = statbotics.Statbotics()
    data = sb.get_team_events(event=event)

    with open(output_path, "w") as f:
        json.dump(data, f, indent=4)


def load_statbotics_teams(filename):
    """
